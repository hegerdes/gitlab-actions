...
if ! command -v vault > /dev/null; then
  ...
  echo "You can set the desired version via VAULT_VERSION. Default is latest"
  ...
  VAULT_DEFAULT_VERSION=$(curl -sL https://api.github.com/repos/hashicorp/vault/releases/latest | jq -r .name)
  VAULT_VERSION=${VAULT_VERSION-$VAULT_DEFAULT_VERSION}
